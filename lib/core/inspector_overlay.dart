...
class _InspectorOverlayLayer extends Layer {
  ...
  ui.Picture _buildPicture(_InspectorOverlayRenderState state) {
    ...
    if (needDescription) {
      _paintDescription(canvas, state.selectionInfo.message, state.textDirection, target, verticalOffset, size, targetRect);
    }
  ...
  void _paintDescription(
    ...
    if (_textPainter == null || textSpan!.text != message || _textPainterMaxWidth != maxWidth) {
      ...
      _textPainter = TextPainter()
        ...
        ..ellipsis = '...'
        ..text = TextSpan(style: const TextStyle(color: kTipTextColor, fontSize: 12.0, height: 1.2), text: message)
        ..textDirection = textDirection
...
class _SelectionInfo {
  ...
  String? get filePath {
    final f = (jsonInfo != null && jsonInfo!.containsKey('creationLocation')) ? jsonInfo!['creationLocation']['file'] : '';
    return f;
  }
...
class _InspectorOverlayRenderState {
  ...
  bool operator ==(dynamic other) {
    ...
    return overlayRect == typedOther.overlayRect && selected == typedOther.selected && listEquals<_TransformedRect>(candidates, typedOther.candidates);
  }
