...
class MemoryService with VMServiceWrapper {
  ...
  void getInfos(Function completion) async {
    List results = await Future.wait([serviceWrapper.getClassHeapStats(), serviceWrapper.getMemoryUsage(), serviceWrapper.getVM()]);
  ...
  void getInstanceIds(String classId, int limit, Function(List<String?>) completion) async {
  ...
  void getClassDetailInfo(String classId, Function(ClsModel?) completion) async {
    ...
    if (cls.fields != null && cls.fields!.isNotEmpty) {
      ...
      cls.fields?.forEach((fieldRef) {
        Property property = Property(fieldRef.isConst, fieldRef.isStatic, fieldRef.isFinal, fieldRef.declaredType!.name, fieldRef.name);
  ...
  void _heapInfoList(List<ClassHeapStats> list) {
    ...
    infoList = allClasses.where((element) => !element.classRef!.name!.startsWith("_")).toList();
  }
  ...
  void sort<T>(Comparable<T>? Function(ClassHeapStats d) getField, bool descending, void Function() completion) {
    s(List list) {
      list.sort((a, b) {
        final aValue = getField(a);
        final bValue = getField(b);
        return descending ? Comparable.compare(bValue!, aValue!) : Comparable.compare(aValue!, bValue!);
      });
    }
